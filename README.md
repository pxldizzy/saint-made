...
Регистрация — `/register`: имя, пароль и **хотя бы один контакт** — e-mail
или телефон. По нему же и вход. Телефон приводится к `+7XXXXXXXXXX`
(принимаются `8 900…`, `+7 900…`, `900…`), поле само держит префикс +7.
Пароль хранится как scrypt-хеш, сессия — подписанная HttpOnly-кука на 8 часов.
